...
├── src/
│   ├── api/                        # API 请求封装
│   │   ├── client.ts               # Axios 实例（baseURL: /api/v1）+ 拦截器
...
│
├── package.json
├── vite.config.ts                  # Vite 配置（路径别名 + Tailwind 插件 + API 代理）
├── tsconfig.json
└── tsconfig.app.json
```
...
已移除 MSW Mock 层，前端通过 Vite 代理直连后端 API。

| 方法 | 路径 | 说明 | 状态 |
|------|------|------|------|
| GET | `/api/v1/meetings` | 分页列表，支持 `?search=&page=&page_size=` 查询 | ✅ 完成 |
| GET | `/api/v1/meetings/:id` | 单条会议详情 | ✅ 完成 |
| POST | `/api/v1/meetings/upload` | 上传文件（multipart/form-data），返回 `{ meeting, task_id }` | ✅ 完成 |
| DELETE | `/api/v1/meetings/:id` | 删除会议 | ✅ 完成 |
| GET | `/api/v1/meetings/:id/export?format=` | 下载导出文件（md/docx） | ✅ 完成 |
| GET | `/api/v1/tasks/:id/status` | 查询任务进度与阶段 | ✅ 完成 |

**后端兼容性：** API 路径与响应格式在 Java（Spring Boot）和 Python（FastAPI）后端之间保持一致，前端无需感知后端类型。
...
2. **Vite 代理对接后端**：开发环境通过 Vite proxy（`/api` → `localhost:8080`）转发请求，生产环境由 Nginx 反向代理
3. **Axios 统一请求封装**：`apiClient` 实例配置 baseURL `/api/v1`，统一错误拦截和消息提示
4. **Zustand 分层 store**：meeting、task、settings 三个独立 store，避免巨型 store 导致的性能问题
5. **设置本地持久化**：settingsStore 使用 zustand/middleware persist 写入 localStorage
6. **任务进度轮询**：上传后 taskStore 启动 1 秒间隔轮询 `/api/v1/tasks/{id}/status`，完成后自动刷新会议列表
...
- 未实现用户认证，无登录/注册功能
- 会议列表刷新依赖轮询完成后手动或自动触发，无 WebSocket 实时推送
- 上传为单文件模式，不支持批量上传
- 摘要页说话人信息展示较弱（仅显示 Speaker_A / Speaker_B 标签）
- 未配置 Electron 桌面端壳
- 无暗色模式 / 多语言支持
...
### 7.1 移除 MSW Mock，对接真实后端

| 变更 | 涉及文件 | 说明 |
|------|----------|------|
| 删除 MSW 依赖 | `package.json` | 移除 `msw` 依赖及 `msw.workerDirectory` 配置 |
| 删除 Mock 文件 | `public/mockServiceWorker.js`、`src/mocks/` 目录 | 清理所有 MSW 相关文件（3 个文件 135 行代码） |
| 简化入口文件 | `src/main.tsx` | 移除 MSW 启动逻辑（已注释的 `worker.start()` 代码块），直接渲染 App |
| 添加 Vite 代理 | `vite.config.ts` | 新增 `server.proxy` 配置：`/api` → `http://localhost:8080` |
...
前端已从 MSW mock 切换到真实后端 API，对接方式：

- **开发环境**：Vite dev server 通过 `server.proxy` 将 `/api` 请求转发到后端（Java 默认 `localhost:8080`，Python 默认 `localhost:8000`）
- **生产环境**：由 Nginx 反向代理统一处理，前端静态资源与后端 API 同域部署
- **API 兼容**：前端 `apiClient` 的 `baseURL` 为 `/api/v1`，所有请求路径与 Java 和 Python 后端完全一致

如需切换后端端口，修改 `vite.config.ts` 中 `server.proxy['/api']` 的目标地址即可，无需改动业务代码。
